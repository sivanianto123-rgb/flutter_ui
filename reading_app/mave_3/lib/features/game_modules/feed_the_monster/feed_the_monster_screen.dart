...
class _FeedTheMonsterScreenState extends ConsumerState<FeedTheMonsterScreen>
  ...
  void initState() {
    super.initState();
    _chompController = AnimationController(vsync: this, duration: const Duration(milliseconds: 320));
    _shakeController = AnimationController(vsync: this, duration: const Duration(milliseconds: 350));
    _bobControllerA  = AnimationController(vsync: this, duration: const Duration(milliseconds: 1400))..repeat(reverse: true);
    _bobControllerB  = AnimationController(vsync: this, duration: const Duration(milliseconds: 1700))..repeat(reverse: true);
    _buildBubbles();
  ...
  Future<void> _playInstruction() async {
    if (!mounted) return;
    try {
      await ref.read(audioServiceProvider).speak('Give me the ${PhoneticHelper.toPhonetic(widget.syllable)}!');
    } catch (_) {}
  }
  ...
  void _onCorrectDrop() {
    ...
    ref.read(audioServiceProvider).speak('Nom nom nom! Yummy!').ignore();
  ...
  void _onWrongDrop() {
    ...
    ref.read(audioServiceProvider).speak('Oops! Try the ${PhoneticHelper.toPhonetic(widget.syllable)} one!').ignore();
  }
                  ...
                  child: DragTarget<bool>(
                    ...
                    builder: (ctx, candidateData, _) {
                      ...
                      return AnimatedBuilder(
                        animation: Listenable.merge([_chompController, _shakeController]),
                        builder: (ctx, child) {
                          final shakeOffset = _shaking && _shakeController.isAnimating
              ...
              if (!_complete)
                Expanded(
                  ...
                  child: Row(
                    ...
                    children: [
                      _BubbleWidget(bubble: _bubbles[0], bobAnim: _bobControllerA, onDrop: _onDrop),
                      _BubbleWidget(bubble: _bubbles[1], bobAnim: _bobControllerB, onDrop: _onDrop),
                    ],
                  ),
                ),
...
class _MonsterWidget extends StatelessWidget {
  const _MonsterWidget({required this.mouthOpen, required this.chompValue, required this.celebrating});
  ...
  Widget build(BuildContext context) => SizedBox(
        ...
        child: CustomPaint(
          painter: _MonsterPainter(mouthOpen: mouthOpen, chompValue: chompValue, celebrating: celebrating),
        ),
      );
}

class _MonsterPainter extends CustomPainter {
  _MonsterPainter({required this.mouthOpen, required this.chompValue, required this.celebrating});
  final bool   mouthOpen;
  final double chompValue;
  final bool   celebrating;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final glowPaint = Paint()
      ..color = celebrating ? const Color(0xFF6BCB77).withOpacity(0.3) : const Color(0xFFFF6B6B).withOpacity(0.25)
      ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 20);
    canvas.drawCircle(Offset(cx, cy), r + 8, glowPaint);

    // Body
    canvas.drawCircle(Offset(cx, cy), r,
        Paint()..color = celebrating ? const Color(0xFF6BCB77) : const Color(0xFFFF6B6B));
    canvas.drawCircle(Offset(cx, cy), r,
        Paint()..color = Colors.white.withOpacity(0.15)..style = PaintingStyle.stroke..strokeWidth = 3);

    // Horns
    final hornPaint = Paint()..color = celebrating ? const Color(0xFF4DB86A) : const Color(0xFFE05555);
    for (final dx in [-r * 0.45, r * 0.45]) {
      canvas.drawOval(
        Rect.fromCenter(center: Offset(cx + dx, cy - r * 0.85), width: 18, height: 30),
        hornPaint,
      );
    }
    ...
    canvas.drawOval(mouthRect, Paint()..color = const Color(0xFF1A0A0A));
    ...
    if (mouthOpen) {
      ...
      for (int i = 0; i < 3; i++) {
        ...
        canvas.drawRRect(
          RRect.fromRectAndRadius(Rect.fromLTWH(tx, mouthRect.top + 2, 10, 12), const Radius.circular(3)),
          toothPaint,
        );
      }
      // Tongue
      canvas.drawOval(
        Rect.fromCenter(center: Offset(cx, mouthRect.bottom - 8), width: r * 0.4, height: 14),
        Paint()..color = const Color(0xFFFF7979),
      );
    }
  }
  ...
  void _drawEye(Canvas canvas, Offset center, double radius) {
    ...
    canvas.drawCircle(center, radius * 0.55, Paint()..color = const Color(0xFF1A0A0A));
    canvas.drawCircle(center + Offset(radius * 0.2, -radius * 0.2), radius * 0.18, Paint()..color = Colors.white);
  }

  @override
  bool shouldRepaint(_MonsterPainter old) =>
      old.mouthOpen != mouthOpen || old.chompValue != chompValue || old.celebrating != celebrating;
}
...
class _BubbleWidget extends StatelessWidget {
  const _BubbleWidget({required this.bubble, required this.bobAnim, required this.onDrop});
  ...
  Widget build(BuildContext context) {
    ...
    final child = Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: color.withOpacity(0.5), blurRadius: 20, offset: const Offset(0, 6))],
      ),
      child: Center(
...
class _StarBurstPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      final x1 = cx + 30 * math.cos(angle);
      final y1 = cy + 30 * math.sin(angle);
      final x2 = cx + 48 * math.cos(angle);
      final y2 = cy + 48 * math.sin(angle);
      canvas.drawLine(Offset(x1, y1), Offset(x2, y2), paint..strokeWidth = 4..strokeCap = StrokeCap.round);
    }
    canvas.drawCircle(Offset(cx, cy), 28, Paint()..color = AppColors.accent);
    canvas.drawCircle(Offset(cx, cy), 28, Paint()..color = Colors.white..style = PaintingStyle.stroke..strokeWidth = 3);
    final tp = TextPainter(
      text: const TextSpan(text: '★', style: TextStyle(fontSize: 28, color: Colors.white)),
